...
def getAdaptiveThresh(frame):
    ...
    adaptiveFrame = cv2.adaptiveThreshold(
        frame, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 51, 7)
...
def getOvalContours(adaptiveFrame, sqrAvrArea):
    contours, hierarchy = cv2.findContours(
        adaptiveFrame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ovalContours = []
    for cnt in contours:
        epsilon = 0.04*cv2.arcLength(cnt, False)
        approx = cv2.approxPolyDP(cnt, epsilon, True)
        ...
        for (q, i) in enumerate(np.arange(0, len(ovalContours), bubbleCount)):
            ...
            for (j, c) in enumerate(bubbles):
                ...
                mask = cv2.bitwise_and(
                    adaptiveWarped, adaptiveWarped, mask=mask)
            ...
            if answer == -1:
                cv2.line(warped, (emptyStartX, emptyStartY),
                         (emptyFinishX, emptyFinishY), (0, 255, 255), 2)
