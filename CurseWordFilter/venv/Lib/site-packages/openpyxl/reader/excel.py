...
class ExcelReader:
    ...
    def read_chartsheet(self, sheet, rel):
        ...
        for rel in drawings:
            charts, images = find_images(self.archive, rel.target_)
            for c in charts:
                cs.add_chart(c)
    ...
    def read_worksheets(self):
        ...
        for sheet, rel in self.parser.find_sheets():
            if rel.target_ not in self.valid_files:
                continue
            ...
            rels_path = get_rels_path(rel.target_)
            rels = RelationshipList()
            if rels_path in self.valid_files:
                rels = get_dependents(self.archive, rels_path)

            if self.read_only:
                ws = ReadOnlyWorksheet(self.wb, sheet.name, rel.target_, self.shared_strings)
                self.wb._sheets.append(ws)
                continue
            else:
                fh = self.archive.open(rel.target_)
            ...
            for r in rels.find(COMMENTS_NS):
                src = self.archive.read(r.target_)
            ...
            if self.wb.vba_archive and ws.legacy_drawing:
                ws.legacy_drawing = rels[ws.legacy_drawing].target_
